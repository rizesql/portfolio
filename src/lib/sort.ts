...
export const byPeriod = (lhs: Period | number, rhs: Period | number) => {
	...
	return getCriteria(rhs).getTime() - getCriteria(lhs).getTime();
};
...
export const dataByPeriod = <T extends SortableData<{ period: Period | number }>>(
	src: T,
) => src.sort((lhs, rhs) => byPeriod(lhs.data.period, rhs.data.period));

export const dataByOrder = <T extends SortableData<{ order: number }>>(src: T) =>
